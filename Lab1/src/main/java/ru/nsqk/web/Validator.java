...
public class Validator {
    ...
    public static String validate(String xStr, String yStr, List<String> rList) {
        ...
        // Ограничение на кол-во знаков после запятой: не более 5 (игнорируя завершающие нули)
        int dot = xStr.indexOf('.');
        if (dot >= 0) {
            String frac = xStr.substring(dot + 1);
            String fracNoTrailingZeros = frac.replaceFirst("0+$", "");
            if (fracNoTrailingZeros.length() > 5) {
                return "X: не более 5 знаков после запятой";
            }
        }
